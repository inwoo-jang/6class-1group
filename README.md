...
각 팀원 폴더에 `AGENTS.md`가 있으면 그 파일의 담당·수정 경계를 먼저 확인합니다.
사람뿐 아니라 팀원이 사용하는 AI 코딩 도구도 같은 규칙을 따라야 합니다.
예를 들어 `src/members/dongyeol/**`와 연결된 `mock-api/dongyeol/**`는 동열의
명시적인 요청과 승인 없이 수정하거나 일괄 리팩터링에 포함하지 않습니다.
...
### 로그인·기록·동열 API Lab 기능을 볼 때

장인우 결과물의 **기록** 메뉴와 동열 결과물의 로그인·상품·게시글 CRUD는
mock 서버가 필요합니다. 창을 하나 더 열고:
...
동열 API는 `/api/dongyeol/**`로 분리되어 기존 API와 겹치지 않습니다.
GitHub Pages에서는 서버 대신 브라우저 저장소를 사용하므로 공개 배포본에서도
동열의 로그인·상품·게시글 실습 흐름을 확인할 수 있습니다. 실제 비밀번호나
민감한 데이터는 입력하지 마세요.
